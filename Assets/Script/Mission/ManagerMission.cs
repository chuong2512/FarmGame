        ...
        switch (Status)
        {
            case 0:
                if (OrderDone[OrderSelected] == true)
                {
                    ...
                    if (TickWordSpace[OrderSelected].activeSelf == true) TickWordSpace[OrderSelected].SetActive(false);
                ...
                for (int j = NumberCombination[i]; j < idItemNow.Length; j++)
                {
                    if (j + 1 < idItemNow.Length)
                    {
                        ...
                        else if (ManagerItem.instance.totalItem[idStypeNow[j]] > 0 && ManagerItem.instance.totalItem[idStypeNow[j + 1]] > 0)
                        {
                            ...
                            PlayerPrefs.SetInt("QuantityItem" + idOrder + "" + 2, mission[idOrder].metarial[2].Amount);
                            NumberCombination[i] += 1;
                            PlayerPrefs.SetInt("NumberCombination" + i, NumberCombination[i]);
                            CalculationCoin(idOrder);
                            CalculationExp(idOrder);
