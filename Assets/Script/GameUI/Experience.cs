...
public class Experience : MonoBehaviour
{
    ...
    [Header("Transform")]
    public Transform PointerExp;
    public Transform PoiterDepot;

    [Header("Array")]
    [SerializeField] Image[] ItemOpen;
    ...
    void Start()
    {
        ...
        if (Application.systemLanguage == SystemLanguage.Vietnamese)
        {
            ...
            for (int i = 2; i < txtNew.Length; i++) { txtNew[i].text = "Mới"; }
        }
        else if (Application.systemLanguage == SystemLanguage.Indonesian)
        {
            txtContinue.text = "Terus";
            for (int i = 2; i < txtNew.Length; i++) { txtNew[i].text = "Baru"; }
        }
        else
        {
            txtContinue.text = "Continue";
            for (int i = 2; i < txtNew.Length; i++) { txtNew[i].text = "New"; }
        }
