        ...
        void Start()
        {
            ...
            if (PlayerPrefs.HasKey("StatusAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal) == true)
            {
                ...
                else if (PlayerPrefs.GetInt("StatusAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal) == 1)
                {
                    ...
                    int time = timeNow -
                               PlayerPrefs.GetInt(
                                   "TimeLastAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal);
                    timelive = PlayerPrefs.GetInt(
                        "TimeLiveAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal) - time;
        ...
        void OnMouseDrag()
        {
                ...
                isClick == true)
            {
                ...
                transform.localScale =
                    new Vector3(transform.localScale.x - 0.0002f, transform.localScale.y - 0.0002f, 1f);
            }
        }
        ...
        void OnMouseUp()
        {
            if (isClick == true)
            {
                ...
                if (Vector3.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
                {
                    ...
                    transform.localScale = new Vector3(transform.localScale.x - 0.0002f,
                        transform.localScale.y - 0.0002f, 1f);
                    string nameItem = Application.systemLanguage == SystemLanguage.Vietnamese
                        ? ManagerData.instance.petCollection.Pet[idBread].detailPet.name
                        : ManagerData.instance.petCollection.Pet[idBread].detailPet.engName;
                    ...
                    if (status == 0)
                    {
                        ...
                        ManagerTool.instance.RegisterShowClock(1, idBread, idAmountBreads, idAmountAnimal, nameItem,
                            target, gameObject);
                        ManagerTool.instance.ShowClockBread(status, timelive, totalTime);
                        if (ManagerGuide.Instance.GuideClickPetsBuyChicken == 0)
                        {
                            ManagerGuide.Instance.GuideClickPetsBuyChicken = 1;
                            ManagerGuide.Instance.DoneArrowPetChicken();
                        }

                        if (idBread == 0 && idAmountBreads == 0 && ManagerGuide.Instance.GuideClickChicken == 0)
        ...
        void OnTriggerEnter2D(Collider2D other)
        {
            if (other.tag == "AnimalFeeds" && ManagerTool.instance.dragging == true)
            {
                if (idBread == ManagerMarket.instance.idAnimalFood && status == 0)
                {
                    ...
                    if (ManagerMarket.instance.QuantityItemFactory[idItemFactory] > 0)
                    {
                        ...
                        PlayerPrefs.SetInt("TimeLiveAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal,
                            timelive);
                        PlayerPrefs.SetInt("TimeLastAnimal" + idBread + "" + idAmountBreads + "" + idAmountAnimal,
                            ManagerGame.Instance.RealTime());
                        StartCoroutine(countTime());
                        Sprite spr = ManagerData.instance.facetoryItems
                            .FacetoryItemDatas[ManagerMarket.instance.idItemFactoryAnimalUse].item;
                        ...
                        if (ManagerGuide.Instance.GuideClickFoodChicken == 0)
                            ManagerGuide.Instance.GuideClickFoodChicken = 1;
                    }
                ...
                ManagerTool.instance.dragging == true)
            {
                if (ManagerMarket.instance.quantityItemDepot < ManagerMarket.instance.quantityTotalItemDepot)
                {
                    ...
                    ManagerMarket.instance.ReciveItem(1, idBread,
                        ManagerData.instance.petCollection.Pet[idBread].detailPet.product, true);
