...
public class HomeAnimal : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("AnimalNow" + idHomeAnimal + "" + idAmountHome))
        {
            ...
            if (animalNow > 0) for (int i = 0; i < animalNow; i++) { Animal[i].SetActive(true); }
        }
        else PlayerPrefs.SetInt("AnimalNow" + idHomeAnimal + "" + idAmountHome, 0);
        SetOrder();
        if (idHomeAnimal == 0 && idAmountHome == 0 && ManagerGuide.instance.GuideClickShopBuyChicken == 0) ManagerGuide.instance.CallArrowShop();
        if (idHomeAnimal == 0 && idAmountHome == 0 && ManagerGuide.instance.GuideClickPetsBuyChicken == 1 && ManagerGuide.instance.GuideClickChicken == 0)
        {
    ...
    public void StartMove()
    {
        ...
        for (int i = 0; i < Animal.Length; i++) { ren[i].sortingLayerName = "Move"; }
    ...
    public void DoneMove()
    {
        ...
        for (int i = 0; i < Animal.Length; i++) { ren[i].sortingLayerName = "Default"; }
        SetOrder();
        Destroy(rgb2D);
    }
    ...
    void OnMouseUp()
    {
        ...
        if (dragging == false)
        {
            if (Vector3.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
            {
                ...
                if (idHomeAnimal == 0 && idAmountHome == 0 && ManagerGuide.instance.GuideClickChicken == 0) ManagerGuide.instance.GuideClickChicken = 1;
    ...
    public void SetOrder()
    {
        ...
        for (int i = 0; i < Animal.Length; i++) { ren[i].sortingOrder = (order + i + 1); }
    }
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.tag == "Pet")
        {
            if (ManagerShop.instance.buyingPet == true)
            {

            }
            else if (ManagerShop.instance.buyingPet == false && ManagerShop.instance.idPet == ManagerShop.instance.idHomeAnimal)
            {
                if (animalNow < Animal.Length)
                {
                    if (ManagerCoin.instance.Coin >= ManagerShop.instance.infoPet.info[idHomeAnimal].goldPrice)
                    {
                        ...
                        if (ManagerGuide.instance.GuideClickPetsBuyChicken == 0 && animalNow == 5) ManagerGuide.instance.GuideClickPetsBuyChicken = 1;
                        if (ManagerGuide.instance.GuideClickPetsBuyChicken == 1 && ManagerGuide.instance.GuideClickChicken == 0)
                        {
                            Vector3 target = Animal[0].transform.position;
                            ManagerGuide.instance.CallArrowDown(target);
                        }
