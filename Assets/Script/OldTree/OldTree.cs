            ...
            if (dragging == false)
            {
                if (Vector3.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        ManagerTool.instance.RegisterShowClock(2, 0, idOldTree, idAmountOldTree, nameItem, target,
                            gameObject);
                        ManagerTool.instance.ShowClockOldTree(timeLive, totalTime);
                        Vector3 targetOne = new Vector3(transform.position.x + 0.7f, transform.position.y + 0.7f,
                    ...
                    else if (status == 1)
                    {
                        ...
                        Vector3 target = new Vector3(transform.position.x - DistanceX, transform.position.y + DistanceY,
                            transform.position.z);
            ...
            if (status == 1 && idOldTree == ManagerTool.instance.idOldTree)
            {
                if (ManagerMarket.instance.quantityItemTower < ManagerMarket.instance.QuantityTotalItemTower)
                {
                    if (isHarvest == true)
                    {
                        ...
                        ManagerMarket.instance.ReciveItem(3, idOldTree,
                            ManagerData.instance.trees.data[idOldTree].Tree.quantity, true);
