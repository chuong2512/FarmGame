...
namespace NongTrai
{
    public class ManagerTower : MonoBehaviour
    {
        ...
        public int levelTower
        {
            get
            {
                if (!PlayerPrefs.HasKey("LevelTower")) PlayerPrefs.SetInt("LevelTower", 0);
                return PlayerPrefs.GetInt("LevelTower");
            }
        ...
        public int capacity
        {
            get
            {
                if (!PlayerPrefs.HasKey("CapacityTower")) PlayerPrefs.SetInt("CapacityTower", 60);
                return PlayerPrefs.GetInt("CapacityTower");
            }
        ...
        void Start()
        {
            if (levelTower < 10)
            {
                ...
                {
                    Destroy(TowerHouse);
                    TowerHouse = Instantiate(ManagerData.instance.tower.Tower[levelTower - 1].depot, transform.position,
                ...
                for (int i = 0; i < DataUpdate.metarial.Length; i++)
                {
                    ...
                    DataUpdate.metarial[i].QuantityItem =
                        ManagerData.instance.tower.Tower[levelTower].metarial[i].QuantityItem;
                }
            }
        ...
        private void CheckEligible()
        {
            ...
            for (int i = 0; i < Condition.Length; i++)
            {
                ...
                {
                    ConditionImage[i].sprite =
                        ManagerData.instance.itemBuildings.Data[DataUpdate.metarial[i].IdItem].Icon;
                    ConditionText[i].text = ManagerMarket.instance.QuantityItemBuilding[DataUpdate.metarial[i].IdItem] +
                                            "/" + DataUpdate.metarial[i].QuantityItem;
                    if (ManagerMarket.instance.QuantityItemBuilding[DataUpdate.metarial[i].IdItem] <
                        DataUpdate.metarial[i].QuantityItem)
                    {
                        if (checkEligible == true) checkEligible = false;
                        if (TickEgiliable[i].activeSelf == true) TickEgiliable[i].SetActive(false);
                        int TotalGem = DataUpdate.metarial[i].QuantityItem -
                                       ManagerMarket.instance.QuantityItemBuilding[DataUpdate.metarial[i].IdItem];
                        ConditinGemText[i].text = "" + TotalGem * ManagerData.instance.itemBuildings
                            .Data[DataUpdate.metarial[i].IdItem].Purchase;
                ...
                {
                    if (Condition[i].activeSelf == true) Condition[i].SetActive(false);
                }
            }
        ...
        public void ButtonYesUpdate()
        {
            ...
            else if (eligible == true)
            {
                for (int i = 0; i < DataUpdate.metarial.Length; i++)
                {
                    ManagerMarket.instance.MinusItem(4, DataUpdate.metarial[i].IdItem,
                        DataUpdate.metarial[i].QuantityItem);
                }
        ...
        public void UseGemUp(int id)
        {
            ...
            if (id == idUseGem)
            {
                ...
                {
                    count = 0;
                    int TotalMiss = DataUpdate.metarial[id].QuantityItem -
                                    ManagerMarket.instance.QuantityItemBuilding[DataUpdate.metarial[id].IdItem];
                    int TotalGem = TotalMiss * ManagerData.instance.itemBuildings.Data[DataUpdate.metarial[id].IdItem]
                        .Purchase;
        ...
        private void UpdateMetarial()
        {
            ...
            else if (levelTower >= 10)
            {
                ...
                for (int i = 0; i < DataUpdate.metarial.Length; i++)
                {
                    ...
                    while (CheckLoop(i, randomIdItem) == false)
                    {
                        randomIdItem = Random.Range(0, 6);
                    }
        ...
        private bool CheckLoop(int possion, int value)
        {
            ...
            {
                if (DataUpdate.metarial[i].IdItem == value)
                {
                    condition = false;
                    break;
                }
