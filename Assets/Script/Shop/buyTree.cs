...
namespace NongTrai
{
    public class buyTree : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IBeginDragHandler,
        IEndDragHandler
    {
                ...
                switch (status)
                {
                    ...
                    case 1:
                        ...
                        if (eventData.pointerEnter == null)
                        {
                            status = 2;
                            Destroy(obj);
                            obj = Instantiate(ManagerShop.instance.Tree[idTree], targetOne, Quaternion.identity,
                                ManagerShop.instance.parentTree[idTree]);
                        ...
                        if (ot.overlap == false)
                        {
                            if (ManagerCoin.Instance.Coin >= ManagerShop.instance.inforTree.info[idTree].goldPrice)
                            {
                                ...
                                PlayerPrefs.SetFloat(
                                    "PosTreeX" + idTree + "" +
                                    ((int) ManagerShop.instance.inforTree.info[idTree].amount - 1),
