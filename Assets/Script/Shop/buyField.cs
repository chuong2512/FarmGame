...
public class buyField : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IBeginDragHandler, IEndDragHandler
{
            ...
            switch (status)
            {
                ...
                case 1:
                    ...
                    if (eventData.pointerEnter == null)
                    {
                        status = 2;
                        Destroy(obj);
                        obj = Instantiate(ManagerShop.instance.Field[idField], targetOne, Quaternion.identity, ManagerShop.instance.parentField[idField]);
                ...
                case 2:
                    ...
                    else if (rg.overlap == false)
                    {
                        if (ManagerCoin.instance.Coin >= ManagerShop.instance.infoField.info[idField].goldPrice)
                        {
                            ...
                            rg.DoneMove();
                            PlayerPrefs.SetFloat("PosFieldX" + ((int)ManagerShop.instance.infoField.info[idField].amount - 1), obj.transform.position.x);
                            PlayerPrefs.SetFloat("PosFieldY" + ((int)ManagerShop.instance.infoField.info[idField].amount - 1), obj.transform.position.y);
                        }
