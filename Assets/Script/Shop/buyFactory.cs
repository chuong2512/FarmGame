...
namespace NongTrai
{
    public class buyFactory : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IBeginDragHandler,
        IEndDragHandler
    {
        private int status;
                ...
                switch (status)
                {
                    ...
                    case 1:
                        ...
                        if (eventData.pointerEnter == null)
                        {
                            status = 2;
                            Destroy(obj);
                            obj = Instantiate(ManagerShop.instance.Factory[idFactory], targetOne, Quaternion.identity,
                                ManagerShop.instance.parentFactory[idFactory]);
                    ...
                    case 2:
                        ...
                        else if (nm.overlap == false)
                        {
                            if (ManagerCoin.Instance.Coin >= ManagerShop.instance.infoFactory.info[idFactory].goldPrice)
                            {
                                ...
                                ManagerBuilding.Instance.RegisterBuilding(1, idFactory, idProduct, time, lighting,
