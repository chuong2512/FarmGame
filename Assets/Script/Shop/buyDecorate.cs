            ...
            switch (status)
            {
                ...
                case 1:
                    ...
                    if (eventData.pointerEnter == null)
                    {
                        status = 2;
                        Destroy(obj);
                        obj = Instantiate(ManagerShop.instance.Decorate[idDecorate], targetOne, Quaternion.identity, ManagerShop.instance.parentDCR[idDecorate]);
        ...
        if (dragging == true)
        {
            ...
            switch (status)
            {
                ...
                case 2:
                    ...
                    {
                        if (ManagerCoin.instance.Coin >= ManagerShop.instance.inforDecorate.info[idDecorate].goldPrice)
                        {
                            dcr.DoneMove();
                            PlayerPrefs.SetFloat("PosDecorateX" + idDecorate + "" + ManagerShop.instance.inforDecorate.info[idDecorate].amount, obj.transform.position.x);
                            PlayerPrefs.SetFloat("PosDecorateY" + idDecorate + "" + ManagerShop.instance.inforDecorate.info[idDecorate].amount, obj.transform.position.y);
                            ManagerShop.instance.buyDecorate(idDecorate);
                        }
                        else if (ManagerCoin.instance.Coin < ManagerShop.instance.inforDecorate.info[idDecorate].goldPrice)
                        {
                            Destroy(obj);
                            string str;
                            if (Application.systemLanguage == SystemLanguage.Vietnamese)
