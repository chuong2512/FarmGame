namespace NongTrai
{
    ...
    public class ManagerBuilding : Singleton<ManagerBuilding>
    {
        ...
        private void Start()
        {
            if (!PlayerPrefs.HasKey("SeriBuilding"))
            {
            ...
            else
            {
                ...
                for (int i = 1; i < SeriBuilding; i++)
                {
                    ...
                    {
                        Vector3 target = new Vector3(PlayerPrefs.GetFloat("PosBuildingX" + i),
                            PlayerPrefs.GetFloat("PosBuildingY" + i), PlayerPrefs.GetFloat("PosBuildingZ" + i));
        ...
        {
            var build = Instantiate(Build, target, Quaternion.identity, transform);
            Building bl = build.GetComponent<Building>();
