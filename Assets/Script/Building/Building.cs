...
public class Building : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        else if (PlayerPrefs.HasKey("StatusBuilding" + seriBuilding) == true)
        {
            ...
            if (status == 0)
            {
                ...
                if (timeLive > 0) { IETimeLive = TimeFinish(); StartCoroutine(IETimeLive); }
                else if (timeLive <= 0)
                {
                    status = 1;
                    PlayerPrefs.SetInt("StatusBuilding" + seriBuilding, status);
        ...
        if (Vector3.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
        {
            switch (status)
            {
                ...
                case 1:
                    ...
                    Instantiate(ManagerBuilding.instance.EffectBuildFinish, transform.position, Quaternion.identity);
