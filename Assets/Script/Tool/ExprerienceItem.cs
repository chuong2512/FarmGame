...
public class ExprerienceItem : MonoBehaviour
{
    ...
    void DrawQuadraticCurveExp()
    {
        for (int i = 1; i < numberPointsExp + 1; i++)
        {
            ...
            positionExp[i - 1] = CalculateQuadraticBezierPoint(t, transform.position, pointerLeft.position, pointerExp.position);
        }
    ...
    void DrawQuadraticCurveItem()
    {
        for (int i = 1; i < numberPointsItem + 1; i++)
        {
            ...
            positionItem[i - 1] = CalculateQuadraticBezierPoint(t, transform.position, pointerRight.position, pointerKho.position);
        }
    }
    ...
    void ExpFly()
    {
        ...
        Exprerience.transform.position = Vector3.MoveTowards(Exprerience.transform.position, positionExp[indexArrayExp], Time.deltaTime * speed);
    ...
    void ItemFly()
    {
        ...
        Item.transform.position = Vector3.MoveTowards(Item.transform.position, positionItem[indexArrayItem], Time.deltaTime * speed);
    }
