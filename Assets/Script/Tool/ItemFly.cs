...
public class ItemFly : MonoBehaviour
{
    ...
    void DrawQuadraticCurveExp()
    {
        for (int i = 1; i < numberPointsItem + 1; i++)
        {
            ...
            positionItem[i - 1] = CalculateQuadraticBezierPoint(t, transform.position, pointerLeft.position, pointerDepot.position);
        }
    ...
    void ExpFly()
    {
        ...
        objItem.transform.position = Vector3.MoveTowards(objItem.transform.position, positionItem[indexArrayExp], Time.deltaTime * speed);
    }
