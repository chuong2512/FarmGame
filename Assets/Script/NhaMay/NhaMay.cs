...
namespace NongTrai
{
    public class NhaMay : MonoBehaviour
    {
                                ...
                                ManagerMarket.instance.quantityTotalItemDepot)
                            {
                                if (CountDone > 1)
                                {
                                    ...
                                    Experience.Instance.registerExp(
                                        ManagerData.instance.facetoryItems.FacetoryItemDatas[idItemDone[CountDone]]
                                            .item,
                                        ManagerData.instance.facetoryItems.FacetoryItemDatas[idItemDone[CountDone]].exp,
                                        ManagerData.instance.facetoryItems.FacetoryItemDatas[idItemDone[CountDone]]
                                            .quantity, IconItemDone[CountDone].transform.position);
                                    ...
                                    if (CountDone == idItemDone.Length - 1 && CountMFT > 0)
                                    {
                                        ...
                                        if (Application.systemLanguage == SystemLanguage.Vietnamese)
                                            nameItemFirst.text = ManagerData.instance.facetoryItems
                                                .FacetoryItemDatas[IdItemMFT[0]].name;
                                        else if (Application.systemLanguage == SystemLanguage.Indonesian)
                                            nameItemFirst.text = ManagerData.instance.facetoryItems
                                                .FacetoryItemDatas[IdItemMFT[0]].nameINS;
                                        else
                                            nameItemFirst.text = ManagerData.instance.facetoryItems
                                                .FacetoryItemDatas[IdItemMFT[0]].engName;
                                        int timeLive = ManagerData.instance.facetoryItems
                                            .FacetoryItemDatas[IdItemMFT[0]].time;
                                ...
                                else if (CountDone == 1)
                                {
                                    ...
                                    Experience.Instance.registerExp(
                                        ...
                                        ManagerData.instance.facetoryItems.FacetoryItemDatas[idItemDone[CountDone]]
                                            .quantity, IconItemDone[CountDone].transform.position);
                                    ManagerMarket.instance.ReciveItem(2, idItemDone[CountDone],
                                        ManagerData.instance.facetoryItems.FacetoryItemDatas[idItemDone[CountDone]]
                                            .quantity, true);
                                    idItemDone[CountDone] = 0;
                                    PlayerPrefs.SetInt("IdItemDone" + idNhaMay + "" + idSoNhaMay + "" + CountDone, 0);
        ...
        {
            if (other.tag == "ItemNhaMay" && ManagerTool.instance.idTypeFactory == idNhaMay &&
                ManagerTool.instance.idFactory == idSoNhaMay)
            {
                if (ManagerTool.instance.dragging == true)
                {
                    ...
                    {
                        itemStay = true;
                        IconItemMFTImage[CountMFT].sprite = ManagerData.instance.facetoryItems
                            .FacetoryItemDatas[ManagerTool.instance.idItem].item;
                        ...
                        if (CountMFT == 0)
                        {
                            if (Application.systemLanguage == SystemLanguage.Vietnamese)
                                nameItemFirst.text = ManagerData.instance.facetoryItems
                                    .FacetoryItemDatas[ManagerTool.instance.idItem].name;
                            else if (Application.systemLanguage == SystemLanguage.Indonesian)
                                nameItemFirst.text = ManagerData.instance.facetoryItems
                                    .FacetoryItemDatas[ManagerTool.instance.idItem].nameINS;
                            else
                                nameItemFirst.text = ManagerData.instance.facetoryItems
                                    .FacetoryItemDatas[ManagerTool.instance.idItem].engName;
                            int timeLive = ManagerData.instance.facetoryItems
                                .FacetoryItemDatas[ManagerTool.instance.idItem].time;
                            timeItemFirst.text = ManagerGame.Instance.TimeText(timeLive);
                        }
                    }
                    else itemStay = true;
                ...
                ManagerTool.instance.idFactory == idSoNhaMay)
            {
                if (ManagerTool.instance.dragging == true)
                {
                    if (itemStay == true)
                    {
                        ...
                        if (CountMFT < NumberMFTFinal)
                        {
                            EmptyMFTText[CountMFT].text = Application.systemLanguage == SystemLanguage.Vietnamese
                                ? "Trống"
                                : "Empty";
                    ...
                    if (itemStay == true)
                    {
                        ...
                        if (CountMFT < NumberMFTFinal)
                        {
                            if (CheckCondition(ManagerTool.instance.idItem) == true)
                            {
                                ...
                                PlayerPrefs.SetInt("IdItemMFT" + idNhaMay + "" + idSoNhaMay + "" + CountMFT,
                                    IdItemMFT[CountMFT]);
                                if (CountMFT == 0)
                                {
                                    int timeLive = ManagerData.instance.facetoryItems.FacetoryItemDatas[IdItemMFT[0]]
                                        .time;
                                    timeItemFirst.text = ManagerGame.Instance.TimeText(timeLive);
                                    LightingText.text = ManagerGame.Instance.CalcalutorGem(timeLive).ToString();
                                    Lighting.SetActive(true);
        ...
        IEnumerator timeLiveFirstItem(int timeLive)
        {
            ...
            else if (timeLive <= 0)
            {
                ...
                IconItemDone[CountDone].sprite =
                    ManagerData.instance.facetoryItems.FacetoryItemDatas[IdItemMFT[0]].item;
                ...
                if (CountMFT > 1)
                {
                    ...
                    for (int i = 0; i < CountMFT + 1; i++)
                    {
                        ...
                        else if (i == CountMFT)
                        {
                            ...
                            EmptyMFTText[i].text = Application.systemLanguage == SystemLanguage.Vietnamese
                                ? "Trống"
                                : "Empty";
                        }
                    }
                    ...
                    if (Application.systemLanguage == SystemLanguage.Vietnamese)
                        ...
                        nameItemFirst.text = ManagerData.instance.facetoryItems.FacetoryItemDatas[IdItemMFT[0]].engName;
                ...
                else if (CountMFT == 1)
                {
                    ...
                    PlayerPrefs.SetInt("CountMFT" + idNhaMay + "" + idSoNhaMay, CountMFT);
                    EmptyMFTText[CountMFT].text =
                        Application.systemLanguage == SystemLanguage.Vietnamese ? "Trống" : "Empty";
                    IconItemMFTImage[CountMFT].color = new Color(1f, 1f, 1f, 0f);
                    if (CountMFT == 0)
                    {
                        nameItemFirst.text = "";
        ...
        private bool CheckCondition(int id)
        {
            ...
            for (int i = 0; i < ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial.Length; i++)
            {
                ...
                {
                    int AmountSeeds =
                        ManagerMarket.instance.QuantityItemSeeds[
                            ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].IdYc];
                    int AmountSeedsRequire =
                        ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].Amount;
                ...
                {
                    int AmountPets =
                        ManagerMarket.instance.QuantityItemPets[
                            ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].IdYc];
                ...
                {
                    int AmountItemFactory =
                        ManagerMarket.instance.QuantityItemFactory[
                            ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].IdYc];
                    int AmountItemFactoryRequire =
                        ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].Amount;
                ...
                {
                    int AmountItemFactory =
                        ManagerMarket.instance.QuantityItemOldTree[
                            ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].IdYc];
                    int AmountItemFactoryRequire =
                        ManagerData.instance.facetoryItems.FacetoryItemDatas[id].metarial[i].Amount;
                    if (AmountItemFactory < AmountItemFactoryRequire) condition = false;
        ...
        public void ButtonLighting()
        {
            ...
            else if (CountConfirmGem == 1)
            {
                ...
                if (ManagerGem.Instance.GemLive >= GemWU)
                {
                    ...
                    PlayerPrefs.SetInt("StatusFactory" + idNhaMay + "" + idSoNhaMay, status);
                    idItemDone[CountDone] = IdItemMFT[0];
                    PlayerPrefs.SetInt("IdItemDone" + idNhaMay + "" + idSoNhaMay + "" + CountDone,
                        idItemDone[CountDone]);
                    IconItemDone[CountDone].sprite =
                        ManagerData.instance.facetoryItems.FacetoryItemDatas[IdItemMFT[0]].item;
                    IconItemDone[CountDone].gameObject.SetActive(true);
                    CountDone += 1;
        ...
        private void InitData()
        {
            ...
            if (PlayerPrefs.HasKey("StatusFactory" + idNhaMay + "" + idSoNhaMay) == true)
            {
                ...
                for (int i = 0; i < NumberMFT; i++)
                {
                    if (i < NumberMFTFinal)
                    {
                        if (IconItemMFTImage[i].gameObject.activeSelf == false)
                            IconItemMFTImage[i].gameObject.SetActive(true);
                    ...
                    {
                        if (IconItemMFTImage[i].gameObject.activeSelf == true)
                            IconItemMFTImage[i].gameObject.SetActive(false);
                ...
                for (int i = 0; i < CountMFT; i++)
                {
                    ...
                    IconItemMFTImage[i].sprite =
                        ManagerData.instance.facetoryItems.FacetoryItemDatas[IdItemMFT[i]].item;
