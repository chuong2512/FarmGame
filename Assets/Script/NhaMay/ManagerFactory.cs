            ...
            for (int i = 0; i < NumberFactory.Length; i++)
            {
                ...
                else if (PlayerPrefs.HasKey("NumberFactory" + i) == true)
                {
                    if (PlayerPrefs.GetInt("NumberFactory" + i) > 0)
                    {
                        ...
                        {
                            Vector2 target = new Vector2(PlayerPrefs.GetFloat("PosFactoryX" + i + "" + j),
                                PlayerPrefs.GetFloat("PosFactoryY" + i + "" + j));
                            GameObject obj = Instantiate(ManagerShop.instance.Factory[i], target, Quaternion.identity,
                                ManagerShop.instance.parentFactory[i]);
