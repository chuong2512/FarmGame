...
public class ItemNhaMay : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        if (ManagerShop.instance.infoItemFactory.info[idItem].status == 1)
        {
            ...
            Vector3 target = new Vector3(Camera.main.ScreenToWorldPoint(Input.mousePosition).x - 1.4f, Camera.main.ScreenToWorldPoint(Input.mousePosition).y, 0);
    ...
    void OnMouseDrag()
    {
        if (unlocked == true)
        {
            ...
            else if (dragging == true)
            {
                ...
                Vector2 targetCam = new Vector2(Camera.main.ScreenToWorldPoint(Input.mousePosition).x - 1f, Camera.main.ScreenToWorldPoint(Input.mousePosition).y + 1f);
