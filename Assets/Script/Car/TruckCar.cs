...
public class TruckCar : MonoBehaviour 
{
	...
	{
		iTween.MoveTo(gameObject, iTween.Hash("position", visuaWP.node [1].position, "time", 6f, "easetype", iTween.EaseType.linear, "islocal", true));
		yield return new WaitForSeconds (6f);
		StartCoroutine (moveCarTwo());
	}
	IEnumerator moveCarTwo()
	{
		iTween.RotateTo(gameObject, iTween.Hash("rotation", new Vector3(0, 0f, 0), "time", 0f, "easetype", iTween.EaseType.linear));
        ...
        iTween.RotateTo(gameObject, iTween.Hash("rotation", new Vector3(0, 180f, 0), "time", 0f, "easetype", iTween.EaseType.linear));
