...
public class Ruong : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (idRuong < 6)
        {
            ...
            {
                transform.position = new Vector2(PlayerPrefs.GetFloat("PosFieldX" + idRuong), PlayerPrefs.GetFloat("PosFieldY" + idRuong));
            }
        ...
        if (idRuong == 0 && ManagerGuide.instance.GuideClickFieldHavestCrop == 0) ManagerGuide.instance.CallArrowDown(transform.position);
        Order();
    }
            ...
            if (Vector2.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
            {
                switch (status)
                {
                    case 0:
                        ...
                        if (ManagerGuide.instance.GuideClickFieldCrop == 0) ManagerGuide.instance.GuideClickFieldCrop = 1;
                        if (ManagerGuide.instance.GuideClickFieldCrop == 1 && ManagerGuide.instance.GuideClickSeeds == 0) ManagerGuide.instance.CallArowSeedsRice();
                        break;
                    case 1:
                        ManagerAudio.instance.PlayAudio(Audio.Click);
                        MainCamera.instance.DisableAll();
                    ...
                    case 2:
                        ...
                        if (ManagerGuide.instance.GuideClickFieldHavestCrop == 0) ManagerGuide.instance.GuideClickFieldHavestCrop = 1;
                        break;
                }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Seeds" && ManagerTool.instance.dragging == true && status == 0)
        {
            if (idRuong == ManagerTool.instance.idRuong)
            {
                ...
                else ManagerUseGem.instance.RegisterUseGemBuySeeds(StypeUseGemBuySeed.SortDay, idSeedNow, gameObject);
            }
