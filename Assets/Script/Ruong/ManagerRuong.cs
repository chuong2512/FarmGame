            ...
            for (int i = 0; i < ManagerShop.instance.Field.Length; i++)
            {
                if (PlayerPrefs.HasKey("amountField" + i) == true)
                {
                    ...
                    if (amountField > 6)
                    {
                        ...
                        {
                            Vector2 target = new Vector2(PlayerPrefs.GetFloat("PosFieldX" + j),
                                PlayerPrefs.GetFloat("PosFieldY" + j));
                            GameObject objField = Instantiate(ManagerShop.instance.Field[i], target,
                                Quaternion.identity, ManagerShop.instance.parentField[i]);
