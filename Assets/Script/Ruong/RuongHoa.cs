    ...
    public class RuongHoa : MonoBehaviour
    {
        ...
        void Start()
        {
            if (PlayerPrefs.HasKey("StatusFieldFlower" + idPOL + "" + idRuongHoa))
            {
                ...
                if (status == 1)
                {
                    ...
                    else if (timeLive <= ManagerData.instance.flowers.Data[idFlower].detailFlower.time * 3 / 4 &&
                             timeLive > ManagerData.instance.seeds.SeedDatas[idFlower].time * 1 / 4)
                        sprRendererCrop.sprite = ManagerData.instance.flowers.Data[idFlower].detailFlower.crop2;
                    else if (timeLive <= ManagerData.instance.flowers.Data[idFlower].detailFlower.time * 1 / 4 &&
                             timeLive > 0)
                ...
                if (Vector2.Distance(camfirstPos, Camera.main.ScreenToWorldPoint(Input.mousePosition)) < 0.2f)
                {
                    switch (status)
                    {
                        ...
                        case 1:
                            ...
                            ManagerTool.instance.RegisterShowClock(4, 0, idPOL, idRuongHoa, nameItem,
        ...
        private void PlantTree()
        {
            if (status == 0)
            {
                ...
                ManagerTool.instance.RegisterEatOne(1,
                    ManagerData.instance.flowers.Data[idFlower].detailFlower.iconStore, transform.position);
            }
        }
